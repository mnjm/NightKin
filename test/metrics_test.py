...
class VRisingMetrics:
    ...
    def __init__(self, url):
        ...
        self.sl_free = 0
        self.cf_free = 0
        self.farbane_occupied = 0
        self.url = url

    def load_data(self) -> bool:
        success = False
        try:
            contents = urlopen(url, timeout=VRisingMetrics.timeout)
    ...
    def write_data_to_FangBotMessage(self):
        ...
        data.append([ "DunleyFarmlands", self.df_free, self.df_free + self.df_occupied ])
        data.append([ "HallowedMountains", self.hm_free, self.hm_free + self.hm_occupied ])
        data.append([ "Gloomrot_South", self.gs_free, self.gs_free + self.gs_occupied ])
        data.append([ "Gloomrot_North", self.gn_free, self.gn_free + self.gn_occupied ])
        data.append([ "CursedForest", self.cf_free, self.cf_free + self.cf_occupied ])
        data.append([ "SilverlightHills", self.sl_free, self.sl_free + self.sl_occupied ])
        ascii = table2ascii(header=header, body=data, first_col_heading=True)
        print(ascii)
